...
export const attempt3Canonical: CanonicalTopologyCandidate = {
  vertices: rawSnapshot.vertices.map((v) => ({
    id: v.canonicalVertexId,
    // Read verbatim from the real snapshot's own coordinate field — never
    // recomputed, never invented. Added for Part B's AI call (see
    // topology-input.ts's CanonicalVertex doc comment); Part A's own logic
    // never reads this field.
    coordinate: { x: v.coordinate.x, y: v.coordinate.y },
  })),
