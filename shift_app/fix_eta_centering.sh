...
class _RunningView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // תיקון סשן 13 (המשך): ירון דיווח שהודעת ה-ETA (ואולי שאר הטקסט כאן)
    // מוצמדת לצד שמאל במקום מרכז המסך במכשיר שלו. בקוד הקודם הסתמכנו על
    // כך ש-Column עם crossAxisAlignment ברירת מחדל (center) ימרכז כל
    // ילד ביחס לרוחב המלא — אבל זה לא בטוח תמיד תלוי בעץ ה-widget
    // שמעל. במקום לנחש, עוטפים כאן במפורש הכל ב-SizedBox ברוחב מלא +
    // crossAxisAlignment מפורש + כל שורת טקסט ברוחב מלא משלה, כדי
    // שהמרכוז יהיה מובטח ולא תלוי בהתנהגות ברירת מחדל.
    return SizedBox(
