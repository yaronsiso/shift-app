...
# SHIFT — סשן 12: תיקון שרת קריטי לבאג "משהו השתבש, הקרדיט לא נוצל"
# אחרי צילום תמונה. האבחון (מלוג Invocations אמיתי ששלח ירון, 400 תוך
# 479ms — כשל ולידציה מיידי, לא כשל AI): השרת עדיין אכף שכל פריט
# חייב להיות מתויג במפורש לחדר שנבחר, בזמן שהלקוח (סשן 10, לפי בקשת
# ירון המפורשת) כבר מציג ומאפשר לבחור כל פריט בכל חדר — כל בחירה
# "לא שגרתית" (למשל פריט חומרי-בניין במרפסת) נכשלה בשרת למרות שהלקוח
# הרשה אותה. בנוסף התגלה שה-dictionary.ts בצד השרת היה **מיושן**:
# עדיין 470 פריטים/10 סוגי חדר (לפני סשן 10) — 14 פריטי המטבח החדשים
# ו-6 סוגי החדר החדשים לא היו קיימים בו בכלל, אז בחירה בהם הייתה
# נכשלת תמיד עם "פריט לא מוכר"/"סוג חדר לא מוכר".
#
# ⚠️ שונה מסשנים 10/11: הקבצים כאן הם בצד ה-**שרת**
# (supabase/functions), לא ב-lib/ של Flutter. אחרי ההרצה חובה גם
# **לפרוס מחדש את ה-Edge Function** (שלב 3 למטה) — flutter build/git
# push לבד לא מספיקים כאן, כי הפונקציה רצה מהקבצים שכבר נפרסו לענן,
# לא מתוך ה-git repo.
#
# מריצים עם: bash /workspaces/shift-app/shift_app/apply_session12.sh

cd /workspaces/shift-app

echo '>> כותב /workspaces/shift-app/shift_app_stage5/supabase/functions/_shared/dictionary.ts'
cat > '/workspaces/shift-app/shift_app_stage5/supabase/functions/_shared/dictionary.ts' << 'SHIFTEOF'
...
echo '>> כותב /workspaces/shift-app/shift_app_stage5/supabase/functions/_shared/prompt_engine.ts'
cat > '/workspaces/shift-app/shift_app_stage5/supabase/functions/_shared/prompt_engine.ts' << 'SHIFTEOF'
...
echo
echo '✅ הקבצים נכתבו בהצלחה (סשן 12).'
echo 'שלבים הבאים — שימו לב, זה כולל פריסה, לא רק git:'
echo '  1) git add -A && git commit -m "סשן 12: תיקון שרת - הסרת הגבלת חדר לפריט + עדכון מילון השרת" && git push'
echo '  2) לוודא של-CLI של Supabase יש נתיב (מסשן קודם — claude/15):'
echo '     ls ~/sbcli/supabase || (mkdir -p ~/sbcli && curl -sL https://github.com/supabase/cli/releases/latest/download/supabase_linux_amd64.tar.gz | tar -xz -C ~/sbcli)'
echo '  3) לפרוס מחדש את הפונקציה — זה השלב הקריטי, בלעדיו התיקון לא באמת עולה:'
echo '     cd /workspaces/shift-app/shift_app_stage5 && ~/sbcli/supabase login   # רק אם נדרש חיבור מחדש'
echo '     cd /workspaces/shift-app/shift_app_stage5 && ~/sbcli/supabase functions deploy generate-render --project-ref iywhxmuzvincfmezijtv --use-api'
echo '  4) בדיקה: לבחור כל חדר + כל חומר, גם קומבינציה לא שגרתית (למשל תריסים במרפסת),'
echo '     לצלם תמונה וללחוץ ליצירה — לוודא שהפעם זה מצליח.'
echo '  אין צורך ב-flutter build/APK חדש הפעם — זה תיקון שרת בלבד, האפליקציה לא משתנה.'
