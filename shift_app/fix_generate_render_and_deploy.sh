...
echo "== שלב 1: מעדכן את קוד הפונקציה generate-render =="
...
git commit -m "fix(generate-render): add safety-net try/catch so failures always surface

Every render row that failed unexpectedly (not one of the known error
branches) was left stuck at status=\"processing\" with error_message=null
forever, and the client only ever saw a generic 502 with no detail —
because nothing wrapped the credit-consumption -> row-insert -> signed-URL
steps in error handling. Any crash there (platform kill, unexpected
exception) skipped every catch block entirely.

Wrap everything after input validation in one outer try/catch: any
unhandled error now gets logged with console.error, marks the render row
as failed with the real error detail, refunds the credit if one was
already consumed, and returns a proper JSON 500 instead of a bare
platform 502. This guarantees the next failure will actually tell us
what happened, instead of leaving a silently stuck row.
...
if ~/sbcli/supabase functions deploy generate-render --project-ref iywhxmuzvincfmezijtv --use-api; then
  echo ""
  echo "✅✅✅ הפונקציה נפרסה בהצלחה! אפשר לבקש מאמא לנסות שוב."
else
  echo ""
  echo "⚠️  הפריסה נכשלה — כנראה צריך להתחבר מחדש ל-Supabase CLI."
  echo "תריץ את הפקודה הזו בנפרד, ותעקוב אחרי ההוראות (היא תיתן לך קישור להיכנס אליו בדפדפן):"
  echo ""
